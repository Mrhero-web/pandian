...
//    CONCAT(IFNULL(a.mingcheng,''),IFNULL(a.bianma,''),IFNULL(a.cunfangdi,''))
    //@Select("SELECT * FROM inventory WHERE CONCAT(IFNULL(mingcheng,''),IFNULL(bianma,''),IFNULL(bumen,''),IFNULL(suoshuxianlu,''),IFNULL(cunfangdi,'')) LIKE '%${value}%'")
    @Select("SELECT a.mingcheng ,a.bianma,r.id,r.money,r.type,r.checkStatus,r.creatTime FROM inventory r, asset a where a.mingcheng LIKE '%${value}%' and r.assetId=a.id ")
    List<AssetInventory> findAll(String conditions);
}
